...
class ImplicitNetworkQFF(nn.Module):
    ...
    def qff_parameters(self):
        print("qff parameters", len(list(self.encoding.parameters())))
        for p in self.encoding.parameters():
            print(p.shape)
        return self.encoding.parameters()
...
class MonoSDFNetwork(nn.Module):
    def __init__(self, conf):
        ...
        QFF_MLP = conf.get_bool('QFF_MLP', default=False)
        self.Grid_MLP = Grid_MLP
        self.QFF_MLP = QFF_MLP
        if QFF_MLP:
            self.implicit_network = ImplicitNetworkQFF(self.feature_vector_size, 0.0 if self.white_bkgd else self.scene_bounding_sphere, **conf.get_config('implicit_network'))    
        elif Grid_MLP:
